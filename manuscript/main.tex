...
%% TODO prose. Establish: the question; why WiFi (ambient, on-vehicle, free TX); the split
%% (localization works / mapping fails) as the headline.
...
%% TODO: passive WiFi radar (indoor/static/infra); ISAC; WiFi-SLAM (fingerprint = infra);
%% range-only SLAM assumptions; the gap this paper occupies.

%% ===========================================================================
\section{Method: One Substrate, Three Sensors, One Testbed}
\subsection{Ray-traced channel and scenes}          %% TODO
\subsection{CSI front-end: joint 2-D (delay--angle) MUSIC}  %% TODO
\subsection{Bistatic-ellipse mapping and particle-filter SLAM}  %% TODO
\subsection{Complexity and real-time feasibility}   %% [R1.4]
%% TODO: 2D-MUSIC + spatial-smoothing cost; the CFAR alternative; on-chip budget
%% (194 kflop/sweep, 26 KB, ~250x headroom on ESP32-S3).
\subsection{LiDAR and 77\,GHz radar baselines}      %% TODO
\subsection{ESP32-S3 hardware testbed}              %% TODO (points to Sec. VI)
...
%% Bibliography activates once sections carry \cite{...} (refs.bib is in place). Until then an
%% empty thebibliography errors, so it is commented for the skeleton stage.

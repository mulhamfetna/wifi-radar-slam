...
%% >>> SLOT (fill once a >=15 m corridor is available; firmware + one-command runner are ready):
%% >>> the phantom rate on a real, tape-measured reflector; MUSIC vs CFAR and monostatic vs
%% >>> bistatic measured physically -- the hardware counterpart of Table~\ref{tab:ablation}. <<<
We stress the scope: this validates the \emph{reading}, not a full SLAM system. It is a static
bench with tape-measured ground truth; we do not claim vehicular mapping from it.

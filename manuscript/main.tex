%% Consolidated journal paper -- IEEE Access submission.
%% Build: pdflatex main; bibtex main; pdflatex main; pdflatex main
%% Class + assets from ACCESS_latex_template_20260513 (ieeeaccess.cls, fonts, spotcolor).
%%
%% ---- reviewer-response map (IoT-J IoT-70358-2026, R1) ----------------------
%% R1.1 novelty -> Sec. I (Contributions) + Sec. II   R1.2 physical validation -> Sec. X
%% R1.3 fragmentation -> this consolidation           R1.4 MUSIC complexity -> Sec. III-E
%% R1.5 landmark/mapping -> Sec. IX                   R1.6/R1.7 robustness -> Sec. IX-F
...
\author{\uppercase{Mulham Fetna}\authorrefmark{1}}
\address[1]{Department of Mechatronics Engineering, University of Aleppo, Aleppo, Syria
(e-mail: contact@mulhamfetna.com)}
\tfootnote{The code, data, and firmware supporting this article are released openly under
AGPL-3.0 at \url{https://github.com/mulhamfetna/wifi-radar-slam} and archived with persistent
DOIs. This article consolidates and extends the author's earlier unpublished feasibility
manuscript, which is cited and publicly available in that repository.}
...
\begin{keywords}
Autonomous vehicles, bistatic radar, channel state information, direction-of-arrival estimation,
millimeter wave radar, multipath channels, passive radar, ray tracing, simultaneous localization
and mapping, wireless fidelity.
\end{keywords}
...
\PARstart{T}{he} exteroceptive sensors that anchor vehicle simultaneous localization and mapping (SLAM) --- LiDAR and radar ---
...
estimator is anchored by vehicle odometry and the bistatic channel state information (CSI)
measurements are
...
frequency-modulated continuous-wave (FMCW) radar. The result is decisive: the radio carrier does not matter, geometry does --- a
...
\emph{assist} a primary ranging sensor (LiDAR or inertial measurement unit, IMU)
...
\textbf{Integrated sensing and communication (ISAC), also called joint communication and
sensing (JCAS),} envisions the same waveform serving
...
Gaussian noise is applied at a configured signal-to-noise ratio (SNR). Radar is electromagnetic, so the same tracer
models it natively; LiDAR requires an optical (diffuse-scattering) treatment
(Sec.~\ref{sec:method}-E).

\subsection{CSI front-end: joint 2-D (delay--angle) MUltiple SIgnal Classification (MUSIC)}
...
(delay--angle) MUSIC} with 2-D spatial smoothing
(singular-value-decomposition, SVD, subspace; delay grid bounded to the
...
to world azimuth by $\sin\beta=-\sin\theta$ for the lateral uniform linear array (ULA).
...
(absolute trajectory error, ATE; relative pose error, RPE) and, for the map, symmetric Chamfer distance, occupancy IoU, and \emph{directional}
...
and a scan-to-map iterative-closest-point (ICP) back-end that is \emph{externally validated} on real KITTI sequences
...
the \emph{excess} delay (relative to the line-of-sight, LOS, arrival) is needed, and that is preserved
...
we parse valid HT40 records ($40$\,MHz, high-throughput, $384$-byte buffers), whose high-throughput long training field (HT-LTF) carries

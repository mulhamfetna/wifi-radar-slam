...
**Status:** **🔒 FROZEN 2026-07-12 — COMPLETE, HELD PENDING PAPER-1 RESOLUTION.**
Pinned at tag **`paper2-v1.0.0`** / branch **`paper2-held`**. All five research questions
answered; manuscript and submission package built and ready. **Do not develop further** —
paper 3 is developed on its own branch off `main`.

This dossier is paper 2's durable, in-repo record — independent of Claude's memory. Read it
first when resuming paper 2.

---

## ⏸️ Why it is held (READ FIRST)

Paper 2 is **finished but not submitted**. It is held until **paper 1 is resolved**, because:

1. **Paper 1 has a self-reported erratum** (`../1-wifi-radar-slam/DOSSIER.md` → *ERRATUM*,
   disclosure letter `erratum-to-editor.md`): its reported realistic joint-MUSIC ATE
   (0.027 m) does **not** reproduce from its own released artifact (actual **0.098 ± 0.028 m**,
   5 seeds). Disclosed to the IoT-J editor 2026-07-12 with a corrected manuscript.
2. Paper 2 should cite a **corrected, published** paper 1 — not an unpublished one under
   review at the same journal, whose interpretation paper 2 also refines.

Paper 2 is nonetheless **self-contained**: every WiFi number is regenerated by its own runs
(`experiments/regen_wifi_results.py`, mean ± std over 5 seeds), so it does **not depend** on
paper 1. The hold protects paper 1's review; it is not a dependency.
...
| | Result |
|---|---|
| **Localization** | WiFi **matches** LiDAR — 0.098 ± 0.028 m vs LiDAR-A 0.102 m (sparse multipath); decimetre-class in general. **Parity, not superiority.** |
| **Mapping** | WiFi fails outright: IoU ≈ 0 vs up to 1.00 for LiDAR. Cost-normalized mapping cost is **infinite**. |
| **Cost** | WiFi package **$40–95** vs the simulated LiDAR tier $8–24 k = **84–600× cheaper**; 4–30 $·m vs 208–2,448 $·m (one-to-two orders better accuracy/dollar). |
| **Fusion** | +0.5 % cost → **36–79 %** better ATE — **conditional on sensor parity**; an 8× regression when badly mismatched. |
| **Mapping ceiling** | **≈89 % phantom detections** + **6.45 m range bias** — *not* path discrimination (2–8 %). A learned discriminator cannot fix it; heuristic, RF and MLP fail identically. |
...
| Tag | Milestone |
|-----|-----------|
| `paper2-v0.1.0` | LiDAR baseline (models A/B/C) + WiFi-vs-LiDAR comparison |
| `paper2-v0.2.0` | Literature/novelty gap + cost model (RQ5) |
| `paper2-v0.3.0` | Fusion (RQ4) |
| `paper2-v0.4.0` | Learned enhancement (RQ2) — negative result + mechanism |
| `paper2-v0.5.0` | Manuscript drafted |
| `paper2-v0.6.0` | Submission package prepared (held) |
| **`paper2-v1.0.0`** | **FREEZE — complete, held** |

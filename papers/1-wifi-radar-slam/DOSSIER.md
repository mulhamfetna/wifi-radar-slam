...
front-end improves realistic commodity-CSI localization ~7x (to 0.098 +- 0.028 m),
approaching but NOT matching the oracle (see ERRATUM). **Mapping** is two-tier: ~25–30 cm with oracle single-bounce
...
- Joint MUSIC (controlled, realistic): ATE 0.73 → **0.098 ± 0.028 m** (5 seeds); the
  oracle is 0.049 ± 0.027 m. (The submitted paper said 0.027 m — see ERRATUM.)
...
- Learned discriminator: F1 1.00 (exact features) → 0.94/0.90/0.86 at (1 m,3°)/(2 m,6°)/(4 m,10°)
  — **but these use an arrival-elevation feature a single-ULA front-end cannot measure**;
  on observable features only, F1 is 0.00–0.45 (see ERRATUM).
